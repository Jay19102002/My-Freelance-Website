...
import { constructOverrides, CONVERSATION_INITIATION_CLIENT_DATA_TYPE, } from "./overrides.js";
import { arrayBufferToBase64 } from "./audio.js";
import { NO_VOLUME } from "./volumeProvider.js";
import { createAudioAdapter, } from "../WebRTCAudioAdapter.js";
...
export class WebRTCConnection extends BaseConnection {
    ...
    input = {
        ...
        getAnalyser: () => this.inputAnalyser,
        getVolume: () => {
            if (this._isMuted)
                return 0;
    ...
    output = {
        ...
        getAnalyser: () => this.outputAnalyser,
    ...
    async setupAudioCapture(track) {
        if (!this.audioAdapter)
            return;
        try {
            const onAudioData = (audioData, maxVolume) => {
            ...
            const result = await this.audioAdapter.setupOutputAnalysis(track, this.outputFormat, onAudioData, this.workletPaths);
    ...
    async setAudioOutputDevice(deviceId) {
        if (!this.audioAdapter) {
            throw new Error("Cannot set output device: no audio adapter available on this platform");
        }
